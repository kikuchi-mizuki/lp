...
def check_user_restriction(line_user_id):
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE user_id = %s AND content_type = 'AIタスクコンシェルジュ'
        ''', (user_id,))
        ...
        return {
            ...
            'message': 'AIタスクコンシェルジュは解約されているため利用できません。' if is_cancelled else '利用可能です。',
            'user_id': user_id,
...
def get_restriction_message():
    ...
    try:
        ...
        if result.get('restricted'):
            ...
            restriction_message = {
                "type": "template",
                "altText": "AIタスクコンシェルジュの利用制限",
                "template": {
                    "type": "buttons",
                    "title": "AIタスクコンシェルジュの利用制限",
                    "text": "AIタスクコンシェルジュは解約されているため、公式LINEを利用できません。\n\nAIコレクションズの公式LINEで再度ご登録いただき、サービスをご利用ください。",
