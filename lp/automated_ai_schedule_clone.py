...
def main():
    ...
    line_channel_id = input("LINEチャネルID（Enterでスキップ）: ")
    line_access_token = input("LINEチャネルアクセストークン（Enterでスキップ）: ")
    line_channel_secret = input("LINEチャネルシークレット（Enterでスキップ）: ")
    ...
    if result['success']:
        print("\n🎉 完全自動複製が成功しました！")
        print(f"📋 企業ID: {result['company_id']}")
        print(f"🌐 デプロイURL: {result['deployment_url']}")
        print(f"🔗 Webhook URL: {result['webhook_url']}")
    else:
        print(f"\n❌ 自動複製に失敗しました: {result['error']}")
